...
int main()
{
	...
	void (*funcPtr[])() = {insertNodeNext, insertNodePrevious, deleteNode, collapseSubNodes, insertSubNode};
		...
		//scanf() returns an int that corresponds to the number of values scanned.
	{
		...
		{
			(*funcPtr[2])(position,originNode);
		}
		else if(instruction == INSERTSUBNODE)
		{
			scanf("%i%i",&subPosition,&value);
			(*funcPtr[4])(position,subPosition,value,originNode);
		}
		else if(instruction == NEXT)
		{
			scanf("%i",&value);
			(*funcPtr[0])(position,value,originNode);
		}
		else if(instruction == PREVIOUS)
		{
			scanf("%i",&value);

			(*funcPtr[1])(position,value,originNode);
		}
		else if (instruction == COLLAPSE){
			(*funcPtr[3])(position,originNode);
		}
	}
